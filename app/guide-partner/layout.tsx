...
import { CalendarCheck, CalendarDays, LayoutDashboard, ListChecks, LogOut, Menu, Plus, X } from "lucide-react";
...
const NAV_ITEMS = [
  ...
  { href: "/guide-partner/listings", label: "Listings", icon: ListChecks },
  { href: "/guide-partner/bookings", label: "Bookings", icon: CalendarCheck },
  { href: "/guide-partner/listings/new", label: "Yangi Listing", icon: Plus },
];
  ...
  return (
    <div className="hl-root flex h-screen bg-slate-50 overflow-hidden text-slate-900">
      <aside className="hidden lg:flex w-[250px] border-r border-slate-200/80 bg-white shrink-0">
        <SidebarContent user={user} initials={initials} isActive={isActive} setDrawerOpen={setDrawerOpen} handleLogout={handleLogout} />
      </aside>
      ...
      {drawerOpen && (
        <div className="fixed inset-0 z-50 flex lg:hidden">
          ...
          <aside className="relative w-[260px] bg-white h-full shadow-2xl flex flex-col">
            <SidebarContent user={user} initials={initials} isActive={isActive} setDrawerOpen={setDrawerOpen} handleLogout={handleLogout} mobile />
          </aside>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="lg:hidden border-t border-slate-200 bg-white px-2 py-2 flex items-center justify-around">
          <Link href="/guide-partner/dashboard" className={`flex flex-col items-center p-1 rounded-xl text-[10px] font-black ${isActive("/guide-partner/dashboard") ? "text-[var(--accent)]" : "text-slate-400"}`}>
            <LayoutDashboard size={20} />
            <span>Home</span>
          </Link>
          <Link href="/guide-partner/listings" className={`flex flex-col items-center p-1 rounded-xl text-[10px] font-black ${isActive("/guide-partner/listings") ? "text-[var(--accent)]" : "text-slate-400"}`}>
            <ListChecks size={20} />
            <span>Listings</span>
          </Link>
          <Link href="/guide-partner/bookings" className={`flex flex-col items-center p-1 rounded-xl text-[10px] font-black ${isActive("/guide-partner/bookings") ? "text-[var(--accent)]" : "text-slate-400"}`}>
