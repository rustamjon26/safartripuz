...
export default function HomeStayPartnerLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    async function loadMe() {
      try {
        ...
        if (data?.user?.role !== "home_stay_partner") {
          router.push("/");
          return;
        }
        setUser(data.user);
  ...
  async function handleLogout() {
    try {
      ...
      // ignore logout errors and continue with redirect
    }
      ...
      <div className="flex items-center gap-3 px-5 py-4 border-b border-slate-200/80 min-h-[64px]">
        ...
        {mobile && (
          <button
            ...
            className="p-1.5 bg-slate-100 rounded-lg text-slate-500"
          >
            <X size={16} />
          </button>
        )}
    ...
    <div className="hl-root flex h-screen bg-slate-50 overflow-hidden text-slate-900">
      ...
      <div className="flex-1 flex flex-col min-w-0">
        <header className="h-[64px] border-b border-slate-200/80 bg-white/80 backdrop-blur-md px-6 flex items-center justify-between shrink-0 z-10 sticky top-0">
          ...
          <Link
            href="/homestay-partner/listings/new"
