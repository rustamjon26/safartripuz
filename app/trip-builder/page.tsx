...
export default function TripBuilderPage() {
  ...
  function ItemCard({ item, isSelected, onToggle }: { item: InventoryItem; isSelected: boolean; onToggle: () => void }) {
    ...
    return (
      ...
      >
        {isSelected && (
          <div className="absolute top-3 right-3 w-7 h-7 bg-amber-500 rounded-full flex items-center justify-center animate-in zoom-in duration-200">
            <Check size={14} strokeWidth={3} className="text-white" />
          </div>
        )}
        <h3 className={`font-black text-sm leading-tight pr-8 mb-1 ${isSelected ? "text-amber-400" : "text-white"}`}>
          {item.title}
        </h3>
        {sub && (
          ...
          {isAdded ? (
              ...
              <div className="flex justify-between items-start mb-1">
                ...
                {onRemove && (
                  <button
                    ...
                    className="text-slate-500 hover:text-red-400 hover:bg-red-500/10 p-1 rounded-md transition-all"
                  >
                    <X size={15} />
                  </button>
                )}
            ...
            <div className="flex items-center justify-between">
              <div className="flex flex-col">
                <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-0.5">{time}</span>
          ...
          <div className="bg-slate-800 border border-amber-500/30 rounded-2xl p-4 flex items-center justify-between shadow-2xl shadow-black/50 backdrop-blur-xl">
            <div>
              ...
              <p className="text-lg font-black text-amber-400">
                {formatUzInteger(grandTotal)} <span className="text-xs text-slate-500">so&apos;m</span>
              </p>
            </div>
            <button
              type="button"
        ...
        {destination && activeTab !== "ai" && weather && (
          <div className="mb-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-2xl flex gap-4 items-start animate-in slide-in-from-top-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 leading-relaxed">{weather.tip}</p>
            </div>
          </div>
        )}
        ...
        {activeTab === "ai" && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 relative">
            ...
            <div className="relative z-10 flex items-center gap-3 mb-6 pt-2">
              ...
              <div>
                <h2 className="text-xl font-black text-white">AI Sayohat Yordamchisi</h2>
                <p className="text-sm text-violet-300/70">Istaganingizni yozing — qolganini men qilaman</p>
              </div>
            </div>
            <form onSubmit={handleAIGenerate} className="relative z-10 mb-5">
              <textarea
                value={aiPrompt}
            ...
            <div className="relative z-10 flex flex-wrap gap-2 mb-5">
              {AI_EXAMPLES.map((ex) => (
                <button
                  ...
                  className="text-xs px-3 py-1.5 rounded-full bg-white/5 border border-white/10 text-slate-400 hover:bg-violet-500/20 hover:text-violet-300 hover:border-violet-500/30 transition-all"
                >
                  {ex}
                </button>
              ))}
            ...
            {aiMessage && (
              <div className="relative z-10 p-4 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 text-sm font-bold flex items-start gap-3 animate-in zoom-in-95">
        ...
        {activeTab === "basics" && (
          <div>
            <h2 className="text-lg font-black text-white mb-5 flex items-center gap-2">
              <MapPin size={18} className="text-amber-400" /> Manzilingizni tanlang
            </h2>
            {loadingDest ? (
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="absolute inset-0 flex flex-col justify-end p-4 text-left">
                        ...
                        <span className="font-black text-white text-sm">{d.title}</span>
                      </div>
                      {selected && (
            ...
            <h2 className="text-lg font-black text-white mb-5 flex items-center gap-2">
              <Calendar size={18} className="text-amber-400" /> Sana va kishilar soni
            </h2>
              ...
              {[
                ...
                <div key={f.label} className="bg-slate-900 border border-slate-700/50 rounded-2xl p-4 hover:border-slate-500 transition-colors">
                  ...
                  <input
                    ...
                    className="w-full bg-transparent font-bold text-white outline-none text-sm cursor-pointer [color-scheme:dark]"
                  />
                </div>
              ))}
            ...
            <div className="flex justify-end">
              <button
                ...
                className="inline-flex items-center gap-2 btn-amber text-white font-bold px-6 py-3 rounded-2xl text-sm"
              >
        ...
        {(activeTab === "hotel" || activeTab === "transport" || activeTab === "guide") && (
          <div>
            <div className="flex items-center justify-between mb-5">
              ...
              <span className="bg-amber-500/15 text-amber-400 text-xs font-black px-3 py-1.5 rounded-full border border-amber-500/20 uppercase tracking-widest">
                {destination}
              </span>
            </div>
      ...
      >
        ...
        >
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-black text-white text-sm">Sayohat Rejasi</h3>
          </div>
          <p className={`text-xs uppercase tracking-widest font-bold ${aiMessage ? "text-violet-200" : "text-slate-500"}`}>
            ...
            {selectedHotel && (
              <div className="flex flex-col gap-2">
                ...
                <div className="flex items-center justify-between pl-1">
                  <span className="text-[10px] text-slate-500 uppercase font-black">{days} tun / xona soni:</span>
                  <input
                    type="number"
                    min={1}
                    value={roomCount}
        ...
        <div className="px-5 pb-5 pt-6">
          <button
            ...
            className="w-full text-xs font-bold text-slate-500 hover:text-amber-400 mb-2 py-1 transition-colors"
          >
            ...
            <div className="bg-slate-900 rounded-xl p-3 mb-3 border border-slate-700/50 space-y-2 text-xs">
              {selectedHotel?.nightlyPrice != null && (
                <div className="flex justify-between text-slate-400">
                  ...
                  <span className="text-white font-bold">{formatUzInteger(hotelTotal)}</span>
                </div>
              )}
              {selectedTaxi && (
                <div className="flex justify-between text-slate-400">
                  <span>🚖 Transport</span>
                  <span className="text-white font-bold">{formatUzInteger(taxiTotal)}</span>
                </div>
              )}
              {selectedGuide?.pricePerDay != null && (
                <div className="flex justify-between text-slate-400">
                  <span>🧭 {formatUzInteger(selectedGuide.pricePerDay)} × {days} kun</span>
                  <span className="text-white font-bold">{formatUzInteger(guideTotal)}</span>
                </div>
              )}
          ...
          <button
            ...
            className="w-full bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 disabled:opacity-30 disabled:cursor-not-allowed text-white font-black py-4 rounded-2xl transition-all shadow-lg shadow-amber-500/20 flex items-center justify-center gap-2 text-sm"
          >
            {submitting ? (
              <>
