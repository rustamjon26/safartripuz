...
import { COLORS, TRAVEL_PLAN_STATUS_COLORS, TRAVEL_PLAN_STATUS_LABELS } from "@/lib/constants";

type Props = { status: string; size?: "sm" | "md" };

export function TravelPlanStatusBadge({ status, size = "md" }: Props) {
  const backgroundColor = TRAVEL_PLAN_STATUS_COLORS[status] ?? COLORS.gray;
  const label = TRAVEL_PLAN_STATUS_LABELS[status] ?? status;
